...
def get_loader(cur_dim, curr_fold, data_file,num_labeled,batch_labeled,batch_unlabeled):
    ...
    dataset_dir = "/mnt/disk1/data0/caihlFiles/deap_shuffled_data/" + cur_dim + "/" #+ "rnn" + "/"

    # cnn_suffix = ".mat_win_128_rnn_dataset.pkl"
    # label_suffix = ".mat_win_128_labels.pkl"
    # dataset_dir = "/mnt/disk1/data0/caihlFiles/seed_shuffled_data/" + "/"  # + "rnn" + "/"
    ...
    # train_data_2 = train_data_1.reshape(-1, 62, 200)
    ...
    train_data_labeled_f = fft.fft(train_data_labeled).abs()
    train_data_labeled_aug1 = DataTransform_TD(train_data_labeled, configs)
    train_data_labeled_f_aug1_f = DataTransform_FD(train_data_labeled_f, configs)
    ...
    train_data_unlabeled_f = fft.fft(train_data_unlabeled).abs()
    train_data_unlabeled_aug1 = DataTransform_TD(train_data_unlabeled, configs)
    train_data_unlabeled_aug1_f = DataTransform_FD(train_data_unlabeled_f, configs)
    ...
    # test_data_2 = test_data_1.reshape(-1, 62, 200)
    ...
    test_data_f = fft.fft(test_data).abs()
    test_data_aug1 = DataTransform_TD(test_data, configs)
    test_data_aug1_f = DataTransform_FD(test_data_f, configs)
    ...
    # #
    train_labeled_dataset = TensorDataset(train_data_labeled, y_train_labeled, train_data_labeled_aug1, train_data_labeled_f, train_data_labeled_f_aug1_f)
    train_labeled_loader = DataLoader(dataset=train_labeled_dataset, batch_size=batch_labeled, shuffle=True)

    train_unlabeled_dataset = TensorDataset(train_data_unlabeled,train_data_unlabeled_aug1,train_data_unlabeled_f,train_data_unlabeled_aug1_f)
    ...
    test_dataset = TensorDataset(test_data, y_test,test_data_aug1,test_data_f,test_data_aug1_f)
...
def get_loader2(dataset, split_idx, num_labeled,batch_labeled,batch_unlabeled):
    ...
    if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
        ...
        train_data_unlabeled = torch.cat((Ptrain_tensor,Pval_tensor),dim=0)
        ...
        # y_test = torch.from_numpy(source_subject_labels[index_test]).type(torch.LongTensor)
        # #
        train_labeled_dataset = TensorDataset(train_data_labeled, y_train_labeled, train_data_labeled_aug1,
                                              train_data_labeled_f, train_data_labeled_f_aug1_f)
        train_labeled_loader = DataLoader(dataset=train_labeled_dataset, batch_size=batch_labeled, shuffle=True)

        train_unlabeled_dataset = TensorDataset(train_data_unlabeled, train_data_unlabeled_aug1, train_data_unlabeled_f,
                                                train_data_unlabeled_aug1_f)
        # train_unlabeled_loader = DataLoader(dataset=train_unlabeled_dataset, batch_size=batch_unlabeled, shuffle=True)
        train_unlabeled_loader = DataLoader(dataset=train_unlabeled_dataset, batch_size=batch_unlabeled, shuffle=True)
...
def get_loader3(sourcedata_path,curr_fold,num_labeled,batch_labeled,batch_unlabeled):
    ...
    train_data_unlabeled_f = fft.fft(unlabeled_train_data).abs()
    train_data_unlabeled_aug1 = DataTransform_TD(unlabeled_train_data, configs)
    train_data_unlabeled_aug1_f = DataTransform_FD(train_data_unlabeled_f, configs)

    # train_data_labeled = torch.from_numpy(labeled_train_data).type(torch.FloatTensor)
    train_data_labeled_f = fft.fft(labeled_train_data).abs()
    train_data_labeled_aug1 = DataTransform_TD(labeled_train_data, configs)
    train_data_labeled_f_aug1_f = DataTransform_FD(train_data_labeled_f, configs)

    test_data_f = fft.fft(test_data_1).abs()
    test_data_aug1 = DataTransform_TD(test_data_1, configs)
    test_data_aug1_f = DataTransform_FD(test_data_f, configs)
    ...
    # y_test = torch.from_numpy(source_subject_labels[index_test]).type(torch.LongTensor)
    # #
    train_labeled_dataset = TensorDataset(labeled_train_data, y_labeled, train_data_labeled_aug1,
                                          train_data_labeled_f, train_data_labeled_f_aug1_f)
    train_labeled_loader = DataLoader(dataset=train_labeled_dataset, batch_size=batch_labeled, shuffle=True)

    train_unlabeled_dataset = TensorDataset(unlabeled_train_data, train_data_unlabeled_aug1, train_data_unlabeled_f,
                                            train_data_unlabeled_aug1_f)
    # train_unlabeled_loader = DataLoader(dataset=train_unlabeled_dataset, batch_size=batch_unlabeled, shuffle=True)
    train_unlabeled_loader = DataLoader(dataset=train_unlabeled_dataset, batch_size=batch_unlabeled, shuffle=True)
    ...
    test_dataset = TensorDataset(test_data_1, y_test_1, test_data_aug1, test_data_f, test_data_aug1_f)
    test_loader = DataLoader(dataset=test_dataset, batch_size=50, shuffle=True)

    # return train_data, y_train, test_data, y_test
    return train_labeled_loader, train_unlabeled_loader, test_loader

def get_loader_mix_all(cur_dim, curr_fold, data_file,num_labeled,batch_labeled,batch_unlabeled):
    cnn_suffix = ".mat_win_128_rnn_dataset.pkl"
    label_suffix = ".mat_win_128_labels.pkl"
    dataset_dir = "/mnt/disk1/data0/caihlFiles/deap_shuffled_data/" + cur_dim + "/" #+ "rnn" + "/"
    ...
    #DEAP dataset  #DEAP number pick out 75% as train 25% as test
    source_subject_data_mix=source_subject_data.reshape(-1,32,128)
    source_subject_labels_mix=source_subject_labels.reshape(-1,)
    index = np.array(range(0, len(source_subject_labels_mix)))
    np.random.shuffle(index)
    source_shuffled_data = source_subject_data_mix[index]
    source_shuffled_label = source_subject_labels_mix[index]
    shuffled_data=source_shuffled_data[:57600]
    shuffled_label=source_shuffled_label[:57600]
    ...
    test_data_2 = source_shuffled_data[57600:]
    # test_data_2 = test_data_1.reshape(-1, 32, 128)
    # test_data_2 = test_data_1.reshape(-1, 62, 200)
    # test_data_2 = test_data_2[:, :1, :]
    # test_data_2 = np.transpose(test_data_1.reshape(-1, 32, 128), (0, 2, 1))
    y_test_2 = source_shuffled_label[57600:]
    # y_test_2 = y_test_1.reshape(-1, )
...
def get_loader_subject_out(cur_dim, curr_fold, data_file,num_labeled,batch_labeled,batch_unlabeled):
    cnn_suffix = ".mat_win_128_rnn_dataset.pkl"
    label_suffix = ".mat_win_128_labels.pkl"
    dataset_dir = "/mnt/disk1/data0/chlFiles/deap_shuffled_data/" + cur_dim + "/" #+ "rnn" + "/"
    ...
    source_subject_data = np.transpose(source_subject_data, (2, 1, 0, 3, 4))

    source_subject_labels = np.transpose(source_subject_labels, (2, 1, 0))

    index = np.array(range(0, 32))
    np.random.shuffle(index)
    shuffled_data = source_subject_data[index]
    shuffled_label = source_subject_labels[index]


    train_data_1=shuffled_data[:30]
    train_data_2=train_data_1.reshape(-1,32,128)
    y_train_1=shuffled_label[:30]
    y_train_2=y_train_1.reshape(-1,)


    labeled_num = int(num_labeled * len(y_train_2))
    labeled_train_data=train_data_2[:labeled_num]
    y_labeled=y_train_2[:labeled_num]
    train_data_labeled = torch.from_numpy(labeled_train_data).type(torch.FloatTensor)
    train_data_labeled_f = fft.fft(train_data_labeled).abs()
    train_data_labeled_aug1 = DataTransform_TD(train_data_labeled, configs)
    train_data_labeled_f_aug1_f = DataTransform_FD(train_data_labeled_f, configs)
    ...
    # unlabeled_train_data=shuffled_data[labeled_num:,:,:]
    unlabeled_train_data = train_data_2
    train_data_unlabeled = torch.from_numpy(unlabeled_train_data).type(torch.FloatTensor)
    train_data_unlabeled_f = fft.fft(train_data_unlabeled).abs()
    train_data_unlabeled_aug1 = DataTransform_TD(train_data_unlabeled, configs)
    train_data_unlabeled_aug1_f = DataTransform_FD(train_data_unlabeled_f, configs)
    ...
    test_data_1 = shuffled_data[30:]
    test_data_2 = test_data_1.reshape(-1, 32, 128)
    y_test_1 = shuffled_label[30:]
    y_test_2 = y_test_1.reshape(-1, )
...
def get_loader_DEAP_SEED(cur_dim, curr_fold, data_file,num_labeled,batch_labeled,batch_unlabeled):
    cnn_suffix = ".mat_win_128_rnn_dataset.pkl"
    label_suffix = ".mat_win_128_labels.pkl"
    dataset_dir1 = "/mnt/disk1/data0/caihlFiles/deap_shuffled_data/" + cur_dim + "/" #+ "rnn" + "/"

    # cnn_suffix = ".mat_win_128_rnn_dataset.pkl"
    # label_suffix = ".mat_win_128_labels.pkl"
    dataset_dir2 = "/mnt/disk1/data0/caihlFiles/seed_shuffled_data/" + "/"  # + "rnn" + "/"

    with open(dataset_dir1 + data_file + cnn_suffix, 'rb') as fp:
        source_subject_data_DEAP = pickle.load(fp)
    with open(dataset_dir1 + data_file + label_suffix, 'rb') as fp:
        source_subject_labels_DEAP = pickle.load(fp)

    with open(dataset_dir2 + data_file + cnn_suffix, 'rb') as fp:
        source_subject_data_SEED = pickle.load(fp)
    with open(dataset_dir2 + data_file + label_suffix, 'rb') as fp:
        source_subject_labels_SEED = pickle.load(fp)

    source_subject_data_SEED = np.transpose(source_subject_data_SEED, (1, 0, 2, 3))
    source_subject_labels_SEED = np.transpose(source_subject_labels_SEED, (1, 0))

    #DEAP dataset
    source_subject_labels = source_subject_labels_DEAP.shape[0]
    indexes_list = np.array([k for k in range(source_subject_labels)])
    index_test = np.array([j for j in range(curr_fold * 10, (curr_fold + 1) * 10)])
    index_train = np.array(list(set(indexes_list) ^ set(index_test)))

    train_data_DEAP=source_subject_data_DEAP[index_train]
    train_data_DEAP=train_data_DEAP.reshape(-1,32,128)
    y_train_DEAP=source_subject_labels_DEAP[index_train]
    y_train_DEAP=y_train_DEAP.reshape(-1,)

    unlabeled_train_data = train_data_DEAP
    train_data_unlabeled = torch.from_numpy(unlabeled_train_data).type(torch.FloatTensor)
    train_data_unlabeled_f = fft.fft(train_data_unlabeled).abs()
    train_data_unlabeled_aug1 = DataTransform_TD(train_data_unlabeled, configs)
    train_data_unlabeled_aug1_f = DataTransform_FD(train_data_unlabeled_f, configs)

    train_data_SEED = source_subject_data_SEED[index_train]
    train_data_SEED = train_data_SEED.reshape(-1, 62, 200)
    train_data_SEED = train_data_SEED[:, :32, :128]
    y_train_SEED = source_subject_labels_SEED[index_train]
    y_train_SEED = y_train_SEED.reshape(-1, )

    train_data_labeled = torch.from_numpy(train_data_SEED).type(torch.FloatTensor)
    train_data_labeled_f = fft.fft(train_data_labeled).abs()
    train_data_labeled_aug1 = DataTransform_TD(train_data_labeled, configs)
    train_data_labeled_f_aug1_f = DataTransform_FD(train_data_labeled_f, configs)

    y_train_labeled=torch.from_numpy(y_train_SEED).type(torch.LongTensor)

    # test_data_DEAP = source_subject_data_DEAP[index_test]
    # test_data_DEAP = test_data_DEAP.reshape(-1, 32, 128)
    test_data_SEED = source_subject_data_SEED[index_test]
    test_data_SEED = test_data_SEED.reshape(-1, 62, 200)
    test_data_SEED = test_data_SEED[:,:32,:128]
    # test_data = np.concatenate([test_data_DEAP,test_data_SEED],axis=0)
    # test_data_2 = test_data_1.reshape(-1, 62, 200)
    # test_data_2 = test_data_2[:, :1, :]
    # test_data_2 = np.transpose(test_data_1.reshape(-1, 32, 128), (0, 2, 1))
    y_test_SEED = source_subject_labels_SEED[index_test]
    y_test_SEED = y_test_SEED.reshape(-1, )
    # y_test_DEAP = source_subject_labels_DEAP[index_test]
    # y_test_DEAP = y_test_DEAP.reshape(-1, )
    # y_test = np.concatenate([y_test_DEAP,y_test_SEED],axis=0)


    # test_data = torch.from_numpy(test_data).type(torch.FloatTensor)
    test_data = torch.from_numpy(test_data_SEED).type(torch.FloatTensor)
    test_data_f = fft.fft(test_data).abs()
    test_data_aug1 = DataTransform_TD(test_data, configs)
    test_data_aug1_f = DataTransform_FD(test_data_f, configs)

    y_test = torch.from_numpy(y_test_SEED).type(torch.LongTensor)
