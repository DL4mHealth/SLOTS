...
from ts2vec import TS2Vec_fine
import tasks
import random
import copy
import datautils
...
if __name__ == '__main__':
    ...
    for looptime in range(loop):
        ...
        for curr_fold in range(fold):
            ...
            train_labeled_loader, train_unlabeled_loader, test_loader = dataset.get_loader_DEAP_SEED(cur_dim, curr_fold,
                                                                                           data_file,
                                                                                           num_labeled, batch_labeled,
                                                                                           batch_unlabeled)
            ...
            loss, performance = model_copy.finetune_fit(train_labeled_loader, test_loader,
                                                      encoding_window='full_series', finetune_epochs=1,
                                                      finetune_lr=0.0001)
    ...
    # print(f"当前维度: {cur_dim}")
        # print(f"平均值：")
        # print(f"Accuracy: {mean(dict_performance['Accuracy'])}")
        # print(f"Accuracy: {mean(dict_performance['Precision'])}")
        # print(f"Accuracy: {mean(dict_performance['Recall'])}")
        # print(f"Accuracy: {mean(dict_performance['F1'])}")
        # print(f"Accuracy: {mean(dict_performance['AUROC'])}")
        # print(f"Accuracy: {mean(dict_performance['AUPRC'])}")
        #
        #
        #
        # # t = time.time() - t
