...
def get_loader2(dataset, split_idx, subset = False):
    ...
    if dataset == 'P12' or dataset == 'P19' or dataset == 'eICU':
        ...
        Ptrain_tensor, Ptrain_static_tensor, Ptrain_time_tensor, ytrain_tensor = tensorize_normalize(Ptrain, ytrain, mf,
                                                                                                     stdf, ms, ss)
        Pval_tensor, Pval_static_tensor, Pval_time_tensor, yval_tensor = tensorize_normalize(Pval, yval, mf, stdf, ms,
                                                                                             ss)
        Ptest_tensor, Ptest_static_tensor, Ptest_time_tensor, ytest_tensor = tensorize_normalize(Ptest, ytest, mf, stdf,
                                                                                                 ms,
                                                                                                 ss)


    train_labeled_dataset = TensorDataset(Pval_tensor, yval_tensor)
    train_labeled_loader = DataLoader(dataset=train_labeled_dataset, batch_size=128, shuffle=True)

    train_unlabeled_dataset = TensorDataset(Ptrain_tensor)
    train_unlabeled_loader = DataLoader(dataset=train_unlabeled_dataset, batch_size=128, shuffle=True)

    # train_dataset = TensorDataset(train_data, y_train)
    # train_loader = DataLoader(dataset=train_dataset, batch_size=100, shuffle=True)

    test_dataset = TensorDataset(Ptest_tensor, ytest_tensor)
