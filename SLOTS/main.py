...
from  model_projection import ProjectionModel
def hotEncoder(v):
...
# def eval_with_linear(model, x):
#     # x=x.cpu()
#     out = model(x)
#     input_dim=out.size(1) * out.size(2)
#     linear1 = nn.Linear(input_dim, 1024)
#     out=linear1(out)
#     # out = F.max_pool1d(
#     #     out.transpose(1, 2),
...
if __name__ == '__main__':
    ...
    model=TSEncoder(input_dims=32, output_dims=1024, hidden_dims=64).to(device)
    ...
    classifier = Classifier(input_dims=128, output_dims=2).to(device)
    ...
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(projection_layer.parameters()) +
                                  list(classifier.parameters()), lr=args.lr, weight_decay=1e-5)
    ...
    for looptime in range(loop):
        ...
        for curr_fold in range(fold):
            ...
            # train_labeled_loader, train_unlabeled_loader, test_loader = dataset.get_loader('P19', curr_fold)
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                for [x_labeled,y_labeled],[x_unlabeled] in zip(train_labeled_loader,train_unlabeled_loader):
                    ...
                    x_l = (1 * x_unlabeled).to(device)
                    x_r = (1 * x_unlabeled).to(device)
                    mask1 = generate_binomial_mask(x_l.size(0), x_l.size(1)).to(device)
                    x_l[~mask1] = 0
                    mask2 = generate_binomial_mask(x_r.size(0), x_r.size(1)).to(device)
                    x_r[~mask2] = 0

                    # plt.figure()
                    # plt.plot(x_unlabeled[5, :, 1].cpu())
                    # plt.show()
                    #
                    # plt.figure()
                    # plt.plot(x_l[5,:,1].cpu())
                    # plt.show()
                    #
                    # plt.figure()
                    # plt.plot(x_r[5, :, 1].cpu())
                    # plt.show()



                    # plt.figure()
                    # plt.plot(x_labeled[2, :, 1].cpu())
                    # plt.show()

                    x_l = x_l.unsqueeze(1)
                    x_r = x_r.unsqueeze(1)

                    out1=model(x_l).to(device)
                    out1=out1.unsqueeze(1)
                    out1 = F.normalize(out1, dim=0)

                    out2 = model(x_r).to(device)
                    ...
                    loss3 = cross_entropy_one_hot(y_pred_prob, label_vec)

                    losstrain = loss1 + loss2 + loss3
                    # losstrain = loss2 + loss3 # W/0 Lu
                    # losstrain = loss1 + loss3  # W/0 Ls
...
#                mean_accuracy / 10))
    ...
    '''掩模后 固定剪裁 不改ECNN模型，网络输出B*Fe  B*1*Fe loss是只使用instance loss'''