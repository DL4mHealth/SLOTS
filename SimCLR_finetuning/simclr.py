...
class SimCLR():
    ...
    def __init__(self, arch='resnet18',out_dim=128,device='cuda',lr=0.0003,weight_decay=1e-4,after_iter_callback=None,
        ...
        self._net = ResNetSimCLR(base_model=arch, out_dim=out_dim)
    ...
    def fit(self, train_unlabeled_loader, n_epochs=None, n_iters=None, verbose=False):
        ...
        while True:
            ...
            for [x_unlabeled] in train_unlabeled_loader:
                ...
                features1 = self._net(aug1).to(self.device)
                features2 = self._net(aug2).to(self.device)
    ...
    def finetune_fit(self, train_labeled_loader, test_loader, mask=None, encoding_window=None,
        ...
        self.proj_head = ProjectionHead(input_dims=128, output_dims=2, hidden_dims=64).to(self.device)#DEAP
        # self.proj_head = ProjectionHead(input_dims=128, output_dims=3, hidden_dims=64).to(self.device)#SEED
        # self.proj_head = ProjectionHead(input_dims=128, output_dims=2, hidden_dims=64).to(self.device)# P19 Epilepsy
        # self.proj_head = ProjectionHead(input_dims=128, output_dims=6, hidden_dims=64).to(self.device)#HAR
        ...
        for epoch in range(finetune_epochs):
            for x, y in train_labeled_loader:
                ...
                # x = x.reshape(x.shape[0], 3, 1, 59)  # Epilepsy HAR
                ...
                features = self.net(x).to(self.device)
                y=y.to(self.device)
